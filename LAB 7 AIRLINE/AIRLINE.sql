...
(SELECT C.AID FROM CERTIFIED C,EMPLOYEES E WHERE C.EID=E.EID AND E.SALARY>=80000);
...
SELECT ENAME FROM EMPLOYEES WHERE EID NOT IN
(SELECT EID FROM CERTIFIED) AND SALARY>(SELECT AVG(SALARY) FROM EMPLOYEES WHERE EID IN(SELECT EID FROM CERTIFIED));